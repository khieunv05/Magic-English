...
paragraphSchema = new mongoose.Schema({
        ...
        point: Number,
        errors: [String],
>>>>>>> 907080a0505c39e2694a8df5a0550ce587032f92
        suggest: String,
        userId: {
...
<<<<<<< HEAD
                res.status(500).json({ message: 'Server gặp lỗi', data: {} });
=======
...
app.post('/api/login', async (req, res) => {
        try {
                ...
                if (!existingUser) {
<<<<<<< HEAD
                        return res.status(404).json({ message: 'Tên đăng nhập không tồn tại', data: {} });
                }
                const isPasswordValid = await bcrypt.compare(password, existingUser.password);
                if (!isPasswordValid) {
                        return res.status(401).json({ message: 'Mật khẩu không đúng', data: {} });
=======
